...
def eval_side(engine, p_world, hand_landmarks, load_kg) -> EvalResult:
    ...
    u_s = engine.get_upper_arm_score(ua, is_abducted_or_raised=is_arm_abducted)
    ...
    flags = {"arm_abd": is_arm_abducted, "wr_dev": is_wrist_deviated, "nk_tw": is_neck_twisted, "tr_tw": is_trunk_twisted}
    return EvalResult(rula_score, tr, la, nk, wr, ua, w_t, flags)


def generate_final_report(res, engine, leg_score, load_kg, worst_img, worst_sec, max_s, fps):
    if not res["sec"]: 
    ...
    step = max(int(fps), 1)
    m_use = 1 if (check_repetition(res["elbow"][::step]) or check_static(res["trunk"][::step])) else 0
...
def analyze_video_per_frame(video_path, load_kg=0, leg_score=1): 
        ...
        while cap.isOpened():
            ...
            if out.pose_world_landmarks and out.pose_landmarks:
                ...
                if not vis_l and not vis_r:
                    ...
                    if has_data and missing_count <= max_hold_frames and last_valid is not None:
                        eval_res = last_valid
                ...
                if eval_res is not None:
                    ...
                    if not is_hold_frame and eval_res.rula_score > max_s:
                        max_s, worst_sec = eval_res.rula_score, curr_time
                        tmp = frame.copy()
                        h, w, _ = frame.shape
    ...
    return generate_final_report(res, engine, leg_score, load_kg, worst_img, worst_sec, max_s, fps)